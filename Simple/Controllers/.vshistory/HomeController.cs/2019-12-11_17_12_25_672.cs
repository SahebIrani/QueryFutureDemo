...
namespace Simple.Controllers
{
	public class HomeController : Controller
	{
		...
		public async Task<IActionResult> IndexAsync(CancellationToken cancellationToken = default)
		{
			...
			ProductResult[] results = await query.OrderBy(p => p.Name).Select(p => new ProductResult(p, query.Count())).Skip(1).Take(4).ToArrayAsync(cancellationToken);
			...
			Task<List<Product>> resultsTask = query.OrderBy(p => p.ProductId).Skip(1).Take(4).ToListAsync(cancellationToken);
