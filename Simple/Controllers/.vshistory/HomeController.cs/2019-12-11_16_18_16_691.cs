...
namespace Simple.Controllers
{
	public class HomeController : Controller
	{
		...
		public async Task<IActionResult> IndexAsync(CancellationToken cancellationToken = default)
		{
			...
			IEnumerable<Product> resultPeopleActiveWithFuture = await Context.Products.AsNoTracking()
				.Where(c => c.IsActive)
				.Future().ToListAsync(cancellationToken)
			;

			IEnumerable<Product> resultPeopleNonActiveWithFuture = await Context.Products.AsNoTracking()
				.Where(c => !c.IsActive)
				.Future().ToListAsync(cancellationToken)
			;
