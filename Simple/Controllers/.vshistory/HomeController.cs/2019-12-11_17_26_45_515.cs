...
namespace Simple.Controllers
{
	public class HomeController : Controller
	{
		...
		public async Task<IActionResult> IndexAsync(CancellationToken cancellationToken = default)
		{
			...
			var pagedQueryable = await query.OrderBy(e => e.ProductId)
				.Skip(1).Take(4)
				.Select(e => new
				{
				...
				.ToListAsync(cancellationToken)
			;
			int total = await pagedQueryable.FirstOrDefault().TotalAsync;
			List<Product> items = pagedQueryable.Select(x => x.Item).ToList();
