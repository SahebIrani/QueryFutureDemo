...
namespace Simple.Controllers
{
	public class HomeController : Controller
	{
		...
		public async Task<IActionResult> IndexAsync(CancellationToken cancellationToken = default)
		{
			...
			IEnumerable<Product> resultPeopleWithFuture = await Context.Products.AsNoTracking()
				.Where(c => c.IsActive)
				.Future().ToListAsync(cancellationToken)
			;

			IEnumerable<Product> resultPeopleWithFuture2 = await Context.Products.AsNoTracking()
				.Where(c => !c.IsActive)
				.Future().ToListAsync(cancellationToken)
			;
