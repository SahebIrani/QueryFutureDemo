	...
	public class HomeController : Controller
	{
		...
		public async Task<IActionResult> IndexAsync(CancellationToken cancellationToken = default)
		{
			...
			QueryFutureValue<int> futureMaxPrice =
				Context.Products.DeferredMax(x => x.Prices).FutureValue();
			...
			QueryFutureValue<Customer> getTotalCount = baseQuery.FutureValue();
			QueryFutureEnumerable<Customer> getPage = baseQuery.Skip(1).Take(4).Future();
			int total = getTotalCount.val;
			var tasks = q2.ToList();
